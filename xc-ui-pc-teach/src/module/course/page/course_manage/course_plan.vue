<template>
  <div>
    ...
    <el-dialog :title="teachPlanName" :visible.sync="teachplayFormVisible" @close="resetForm">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editTeachplan(data) {
        courseApi.getCourseTeachPlan(data.id).then(res => {
          if (res.success) {
            //成功
            this.teachplanActive = res.queryResult.list[0]
          }
        })
      },
      edit() {
      ...
      remove(node, data) {
        return new Promise((resolve, reject) => {
          courseApi.delCourseTeachPlan(node.key).then(res => {
            if (res.success) {
